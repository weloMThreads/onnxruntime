...
// Use the lightweight .mu-compatible header (no ORT headers, no C++17)
#include "core/providers/musa/tensor/resize_impl_mu.h"

#ifndef CUDA_LONG
#define CUDA_LONG int32_t
#endif

// ORT_THROW replacement for .mu files
#define ORT_THROW(...) do { /* no-op in device code */ } while(0)
...
struct NearestPixel_SIMPLE {
  __device__ __forceinline__ int operator()(float x_original, bool is_down_sampling) const {
    if (is_down_sampling) {
      return static_cast<int>(ceilf(x_original));
    }
...
struct NearestPixel_ROUND_PREFER_FLOOR {
  __device__ __forceinline__ int operator()(float x_original, bool) const {
    if (x_original == static_cast<int>(x_original) + 0.5f) {
      return static_cast<int>(floorf(x_original));
    }
...
template <typename T>
__global__ void _ResizeBilinearKernelVecNchwC5(
    int64_t input_height, int64_t input_width,
    int64_t output_height, int64_t output_width,
    fast_divmod div_output_width,
    const T* input_data, T* output_data, const size_t N_per_channel,
    const T extrapolation_value,
    const LinearMappingInfo* dims_mapping) {
  CALCULATE_ELEMENTWISE_INDEX_OR_EXIT(id, N_per_channel);

  const int output_plane_stride = static_cast<int>(output_height * output_width);
  const int batch_idx = id / output_plane_stride;
  const int output_image_index = id - batch_idx * output_plane_stride;
  int output_y, output_x;
  div_output_width.divmod(output_image_index, output_y, output_x);

  const LinearMappingInfo& y_mapping = dims_mapping[output_y];
  const LinearMappingInfo& x_mapping = dims_mapping[output_x + output_height];

  const int64_t input_channel_stride = input_height * input_width;
  const int64_t output_channel_stride = output_height * output_width;
  const int64_t input_batch_base = static_cast<int64_t>(batch_idx) * 5 * input_channel_stride;
  const int64_t output_batch_base = static_cast<int64_t>(batch_idx) * 5 * output_channel_stride;

  if (y_mapping.extrapolate_ || x_mapping.extrapolate_) {
#pragma unroll
    for (int c = 0; c < 5; ++c) {
      output_data[output_batch_base + static_cast<int64_t>(c) * output_channel_stride + output_image_index] =
          extrapolation_value;
    }
    return;
  }

  const float y_offset_0 = y_mapping.weight_;
  const int y_int = y_mapping.origin_;
  const float x_offset_0 = x_mapping.weight_;
  const int x_int = x_mapping.origin_;
  const int64_t input_index = static_cast<int64_t>(y_int) * input_width + x_int;

  const bool end_of_h = (y_int >= input_height - 1);
  const bool end_of_w = (x_int >= input_width - 1);
  const int64_t input_h_stride = end_of_h ? 0 : input_width;
  const int64_t input_w_stride = end_of_w ? 0 : 1;

  const float y_offset_1 = 1.0f - y_offset_0;
  const float x_offset_1 = 1.0f - x_offset_0;

#pragma unroll
  for (int c = 0; c < 5; ++c) {
    const int64_t input_base = input_batch_base + static_cast<int64_t>(c) * input_channel_stride + input_index;
    const T x00 = input_data[input_base];
    const T x10 = input_data[input_base + input_w_stride];
    const T x01 = input_data[input_base + input_h_stride];
    const T x11 = input_data[input_base + input_h_stride + input_w_stride];
...
__global__ void _ResizeCubicCoordinateMapping(
  ...
  int coord_int = static_cast<int>(floorf(input_coordinat));
...
size_t CalcResizeBufferSize(const onnxruntime::UpsampleMode upsample_mode,
  ...
  switch (upsample_mode) {
    case UpsampleMode::NN:
      return sizeof(int64_t) * rank +
             sizeof(NearestMappingInfo) *
                 static_cast<size_t>(std::accumulate(output_dims, output_dims + rank, (int64_t)0));
    case UpsampleMode::LINEAR:
      return sizeof(LinearMappingInfo) *
             static_cast<size_t>(std::accumulate(output_dims + rank - 2, output_dims + rank, (int64_t)0));
...
void ResizeImpl(
  ...
  const bool disable_fast_path = std::getenv("MUSA_RESIZE_DISABLE_FASTPATH") != nullptr;
  ...
  switch (upsample_mode) {
    case UpsampleMode::LINEAR:
      if (is_2D) {
        ...
        if (!disable_fast_path && rank == 4 && !extrapolation_enabled && input_shape[rank - 3] == 5) {
          const size_t N_per_channel = N / 5;
          const int blocksPerGridVec = static_cast<int>(
              ceil(static_cast<float>(N_per_channel) / GridDim::maxThreadsPerBlock));
          _ResizeBilinearKernelVecNchwC5<T><<<blocksPerGridVec, GridDim::maxThreadsPerBlock, 0, stream>>>(
              input_shape[rank - 2], input_shape[rank - 1],
              output_height, output_width,
              output_div_pitches[rank - 2],
              input_data, output_data, N_per_channel,
              extrapolation_value,
              reinterpret_cast<LinearMappingInfo*>(dims_mapping));
          return;
        }
...
void ResizeImplHalf(
    ...
    ResizeNearestMode nearest_mode,
    void* dims_mapping) {
  half extrap_val = __float2half(extrapolation_value);
  ResizeImpl<half>(stream, upsample_mode, rank, input_shape, output_shape,
                   input_strides, output_div_pitches, scales_vals, roi_vals,
                   static_cast<const half*>(input_data),
                   static_cast<half*>(output_data),
                   N, use_extrapolation, extrap_val,
                   cubic_coeff_a, exclude_outside,
                   coordinate_transform_mode, nearest_mode, dims_mapping);
}
