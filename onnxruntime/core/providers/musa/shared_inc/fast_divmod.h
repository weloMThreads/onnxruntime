...
//
// Ported to MUSA by Moore Threads, 2026.
//
...
// then GPU compiler can do loop unroll easily when divmod is called in a loop.
template <>
struct DivMod<int> {
  DivMod(int d = 1) {
    d_ = d == 0 ? 1 : d;
  ...
  __host__ __device__ inline int div(int n) const {
    ...
    uint32_t t = __umulhi(M_, static_cast<uint32_t>(n));
    return static_cast<int>((t + static_cast<uint32_t>(n)) >> l_);
#else
    // Using uint64_t for t, then t + n won't overflow.
    uint64_t t = (static_cast<uint64_t>(M_) * static_cast<uint32_t>(n)) >> 32;
    return static_cast<int>((t + static_cast<uint32_t>(n)) >> l_);
#endif
  }
