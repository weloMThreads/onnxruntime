...
#include "core/providers/musa/math/cos.h"
...
// MUSA device-based cos implementation using mudnn Unary class
template <typename T>
Status SimpleMusaCosOp(const MusaPreparation& prepare) {
  ...
  try {
    ...
    // Set the operation mode to COS
    auto status = unary_op.SetMode(::musa::dnn::Unary::Mode::COS);
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "Failed to set mudnn Unary mode to COS");
    }
    ...
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "mudnn Unary Cos operation failed, status: " +
                             std::to_string(static_cast<int>(status)));
    }

  } catch (const std::exception& e) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "Exception in mudnn Unary Cos operation: " +
                           std::string(e.what()));
  }
...
// Prepare method - reuse similar pattern from exp
template <typename T>
Status Cos<T>::Prepare(OpKernelContext* ctx, MusaPreparation& prepare) const {
  // 1. Get input tensor
...
template <typename T>
Status Cos<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_RETURN_IF_ERROR(SimpleMusaCosOp<T>(prepare));
  return Status::OK();
}

// Explicit template instantiations for supported types - 不支持double和bfloat16
template class Cos<float>;
template class Cos<MLFloat16>;

// Register kernels for different ONNX versions
#define REGISTER_MUSA_COS_TYPED_KERNEL(ver, T)                          \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                        \
      Cos, kOnnxDomain, ver, T, kMusaExecutionProvider,                 \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Cos<T>);

#define REGISTER_MUSA_COS_VERSIONED_TYPED_KERNEL(startver, endver, T)   \
  ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(                              \
      Cos, kOnnxDomain, startver, endver, T, kMusaExecutionProvider,    \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Cos<T>);

// Register versions 7-12 (根据ONNX标准)
REGISTER_MUSA_COS_VERSIONED_TYPED_KERNEL(7, 12, float)
REGISTER_MUSA_COS_VERSIONED_TYPED_KERNEL(7, 12, MLFloat16)

// Register version 13+ (current)
REGISTER_MUSA_COS_TYPED_KERNEL(13, float)
REGISTER_MUSA_COS_TYPED_KERNEL(13, MLFloat16)
