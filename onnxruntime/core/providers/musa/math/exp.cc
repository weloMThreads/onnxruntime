...
#include "core/providers/musa/math/exp.h"
...
// MUSA device-based exp implementation using mudnn Unary class
template <typename T>
Status SimpleMusaExpOp(const MusaPreparation& prepare) {
  ...
  try {
    ...
    // Set the operation mode to EXP
    auto status = unary_op.SetMode(::musa::dnn::Unary::Mode::EXP);
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "Failed to set mudnn Unary mode to EXP");
    }
    ...
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "mudnn Unary Exp operation failed, status: " +
                             std::to_string(static_cast<int>(status)));
    }

  } catch (const std::exception& e) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "Exception in mudnn Unary Exp operation: " +
                           std::string(e.what()));
  }
...
// Prepare method - reuse similar pattern from abs
template <typename T>
Status Exp<T>::Prepare(OpKernelContext* ctx, MusaPreparation& prepare) const {
  // 1. Get input tensor
...
template <typename T>
Status Exp<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_RETURN_IF_ERROR(SimpleMusaExpOp<T>(prepare));
  return Status::OK();
}

// Explicit template instantiations for supported types - 不支持double和bfloat16
template class Exp<float>;
template class Exp<MLFloat16>;

// Register kernels for different ONNX versions
#define REGISTER_MUSA_EXP_TYPED_KERNEL(ver, T)                          \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                        \
      Exp, kOnnxDomain, ver, T, kMusaExecutionProvider,                 \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Exp<T>);

#define REGISTER_MUSA_EXP_VERSIONED_TYPED_KERNEL(startver, endver, T)   \
  ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(                              \
      Exp, kOnnxDomain, startver, endver, T, kMusaExecutionProvider,    \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Exp<T>);

// Register versions 6-12 (根据CUDA EP的模式)
REGISTER_MUSA_EXP_VERSIONED_TYPED_KERNEL(6, 12, float)
REGISTER_MUSA_EXP_VERSIONED_TYPED_KERNEL(6, 12, MLFloat16)

// Register version 13+ (current)
REGISTER_MUSA_EXP_TYPED_KERNEL(13, float)
REGISTER_MUSA_EXP_TYPED_KERNEL(13, MLFloat16)
