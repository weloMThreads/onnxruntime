...
#include "core/providers/musa/math/less_or_equal.h"
...
Status SimpleMusaLessOrEqualOp(const MusaPreparation& prepare, size_t size) {
  ...
  try {
    ...
    // Set the operation mode to LE (Less or Equal)
    auto status = binary_op.SetMode(::musa::dnn::Binary::Mode::LE);
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "Failed to set mudnn Binary mode to LE");
    }
    ...
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "mudnn Binary LessOrEqual operation failed, status: " +
                             std::to_string(static_cast<int>(status)));
    }

  } catch (const std::exception& e) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "Exception in mudnn Binary LessOrEqual operation: " +
                           std::string(e.what()));
  }
...
template <typename T>
template <typename U>
Status LessOrEqual<T>::Prepare(OpKernelContext* ctx, MusaPreparation& prepare) const {
  ...
  // 3. Create output tensor - LessOrEqual always outputs bool type
...
#define REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(T)                                   \
  template <>                                                                  \
  Status LessOrEqual<T>::ComputeInternal(OpKernelContext* ctx) const {               \
    ...
    ORT_RETURN_IF_ERROR(SimpleMusaLessOrEqualOp<T>(prepare, prepare.output_size));  \
    return Status::OK();                                                       \
  }

// Macro for registering typed kernel
#define REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL(ver, T)                               \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                               \
      LessOrEqual, kOnnxDomain, ver, T, kMusaExecutionProvider,                      \
...
#define REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED_KERNEL(startver, endver, T)        \
  ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(                                     \
      LessOrEqual, kOnnxDomain, startver, endver, T, kMusaExecutionProvider,         \
      (*KernelDefBuilder::Create())                                            \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>())               \
          .TypeConstraint("T1", DataTypeImpl::GetTensorType<bool>()),          \
      LessOrEqual<T>);
...
#define REGISTER_MUSA_LESS_OR_EQUAL_TYPED(ver, T)                                      \
  REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL(ver, T)                                     \
  REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(T)

// Combined macro for versioned registration (kernel only, no compute)
#define REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(startver, endver, T)               \
  REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED_KERNEL(startver, endver, T)

// Register LessOrEqual operations for supported data types
...
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, bool)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, int8_t)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, uint8_t)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, int16_t)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, int32_t)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, int64_t)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, MLFloat16)
REGISTER_MUSA_LESS_OR_EQUAL_VERSIONED_TYPED(12, 15, float)

// Register typed kernels for later versions (kernel only)
#define REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(ver, T) \
  REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL(ver, T)

REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(16, bool)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(16, int8_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(16, uint8_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(16, int16_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_KERNEL_ONLY(16, int32_t)
...
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(int8_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(uint8_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(int16_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(int32_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(int64_t)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(MLFloat16)
REGISTER_MUSA_LESS_OR_EQUAL_TYPED_COMPUTE(float)
