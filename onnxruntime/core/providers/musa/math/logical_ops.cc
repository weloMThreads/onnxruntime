...
#include "core/providers/musa/math/logical_ops.h"
...
Status BuildLogicalBinaryParams(const TensorShape& lhs_shape,
                                const TensorShape& rhs_shape,
                                const TensorShape& output_shape,
                                LogicalBinaryParams* params) {
  ORT_RETURN_IF_NOT(params != nullptr, "LogicalBinaryParams must not be null");

  const int32_t rank = static_cast<int32_t>(output_shape.NumDimensions());
  ORT_RETURN_IF_NOT(rank <= kLogicalBinaryMaxDims,
                    "Logical binary ops support rank up to ", kLogicalBinaryMaxDims,
                    ", got ", rank);

  params->rank = rank;
  params->total_elements = output_shape.Size();
  std::fill_n(params->output_strides, kLogicalBinaryMaxDims, 0);
  std::fill_n(params->lhs_strides, kLogicalBinaryMaxDims, 0);
  std::fill_n(params->rhs_strides, kLogicalBinaryMaxDims, 0);
...
Status CopyHostBoolInputIfNeeded(const void* input,
  ...
  *temp_buffer = nullptr;
  ...
  const size_t bytes = static_cast<size_t>(shape.Size()) * sizeof(bool);
  if (bytes == 0) {
    return Status::OK();
  }
...
Status LogicalBinary<Op>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  LogicalBinaryParams params{};
  ORT_RETURN_IF_ERROR(BuildLogicalBinaryParams(A->Shape(), B->Shape(), output_shape, &params));
  ...
  ORT_RETURN_IF_ERROR(CopyHostBoolInputIfNeeded(A->DataRaw(), A->Shape(), stream, &device_a, &temp_a));
  ORT_RETURN_IF_ERROR(CopyHostBoolInputIfNeeded(B->DataRaw(), B->Shape(), stream, &device_b, &temp_b));

  LaunchLogicalBinaryKernel(stream,
                            reinterpret_cast<const bool*>(device_a),
                            reinterpret_cast<const bool*>(device_b),
                            reinterpret_cast<bool*>(C->MutableDataRaw()),
                            params,
                            Op);
...
template class LogicalBinary<LogicalOpType::And>;
template class LogicalBinary<LogicalOpType::Or>;
template class LogicalBinary<LogicalOpType::Xor>;

#define REGISTER_MUSA_LOGICAL_BINARY_TYPED_KERNEL(OpName, Impl)               \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                              \
      OpName, kOnnxDomain, 7, bool, kMusaExecutionProvider,                   \
