...
#include "core/providers/musa/math/sin.h"
...
// MUSA device-based sin implementation using mudnn Unary class
template <typename T>
Status SimpleMusaSinOp(const MusaPreparation& prepare) {
  ...
  try {
    ...
    // Set the operation mode to SIN
    auto status = unary_op.SetMode(::musa::dnn::Unary::Mode::SIN);
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "Failed to set mudnn Unary mode to SIN");
    }
    ...
    if (status != ::musa::dnn::Status::SUCCESS) {
      return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                             "mudnn Unary Sin operation failed, status: " +
                             std::to_string(static_cast<int>(status)));
    }

  } catch (const std::exception& e) {
    return ORT_MAKE_STATUS(ONNXRUNTIME, FAIL,
                           "Exception in mudnn Unary Sin operation: " +
                           std::string(e.what()));
  }
...
template <typename T>
Status Sin<T>::Prepare(OpKernelContext* ctx, MusaPreparation& prepare) const {
  // 1. Get input tensor
...
template <typename T>
Status Sin<T>::ComputeInternal(OpKernelContext* ctx) const {
  ...
  ORT_RETURN_IF_ERROR(SimpleMusaSinOp<T>(prepare));
  return Status::OK();
}

// Explicit template instantiations for supported types - 不支持double和bfloat16
template class Sin<float>;
template class Sin<MLFloat16>;

// Register kernels for different ONNX versions
#define REGISTER_MUSA_SIN_TYPED_KERNEL(ver, T)                          \
  ONNX_OPERATOR_TYPED_KERNEL_EX(                                        \
      Sin, kOnnxDomain, ver, T, kMusaExecutionProvider,                 \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Sin<T>);

#define REGISTER_MUSA_SIN_VERSIONED_TYPED_KERNEL(startver, endver, T)   \
  ONNX_OPERATOR_VERSIONED_TYPED_KERNEL_EX(                              \
      Sin, kOnnxDomain, startver, endver, T, kMusaExecutionProvider,    \
      (*KernelDefBuilder::Create())                                     \
          .TypeConstraint("T", DataTypeImpl::GetTensorType<T>()),       \
      Sin<T>);

// Register version 7+ (based on CUDA EP pattern: UNARY_OP_HFD(Sin, 7))
REGISTER_MUSA_SIN_TYPED_KERNEL(7, float)
REGISTER_MUSA_SIN_TYPED_KERNEL(7, MLFloat16)
