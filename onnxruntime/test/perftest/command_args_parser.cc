...
ABSL_FLAG(std::string, e, "cpu", "Specifies the provider 'cpu','cuda','musa','dnnl','tensorrt', 'nvtensorrtrtx', 'openvino', 'dml', 'acl', 'nnapi', 'coreml', 'qnn', 'snpe', 'migraphx', 'xnnpack', 'vitisai' or 'webgpu'.");
  ...
  // -e
  {
    ...
    if (!ep.empty()) {
      if (ep == "cpu") {
        ...
        test_config.machine_config.provider_type_name = onnxruntime::kMusaExecutionProvider;
      } else if (ep == "dnnl") {
